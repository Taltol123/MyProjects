...
class Perceptron(BaseEstimator):
    ...
    def checking_for_misclassified_sample(self, X: np.ndarray, y: np.ndarray):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.include_intercept_:
            X = np.c_[np.ones(X_size), X]

        self.coefs_ = np.zeros(X.shape[1])

        while curr != self.max_iter_:
            curr += 1
            is_exist, i = self.checking_for_misclassified_sample(X, y)
            if is_exist:
                self.coefs_ += y[i] * X[i]
        ...
        self.callback_(self, np.nan, np.nan)
