...
def split_train_test(X: pd.DataFrame, y: pd.Series, train_proportion: float = .75) \
    ...
    number_of_train_samples = int(np.ceil(train_proportion * number_of_samples))

    shuffled_samples_order = np.random.permutation(number_of_samples)

    train_indexes = shuffled_samples_order[:number_of_train_samples]
    test_indexes =  shuffled_samples_order[number_of_train_samples:]
