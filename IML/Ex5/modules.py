...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.inner(self.weights_, self.weights_)
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        n_samples = X.shape[0]
        fw = - (X @ self.weights @ y - np.log(1 + np.exp(X @ self.weights)).sum()) / n_samples
        return fw
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        n_samples = X.shape[0]
        s = 1 / (1 + (1/(np.exp(X @ self.weights))))
        return -((y - s)@X)/ n_samples
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        fw = self.fidelity_module_.compute_output(**kwargs) + self.lam_ * self.regularization_module_.compute_output(
            **kwargs)
        return fw
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        reg_jacobian = self.lam_ * self.regularization_module_.compute_jacobian(**kwargs)
