...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        initialize_weights = np.random.normal(0, 1, X.shape[1]) / np.sqrt(X.shape[1])
        module = self._get_model(initialize_weights)
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        predict = [1 if p > self.alpha_ else 0 for p in proba_predict]
        return predict
