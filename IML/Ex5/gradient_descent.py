...
class GradientDescent:
    ...
    def _best_w_calc(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        w_sum = f.weights
        best_w = f.weights
        ...
        for t in range(1, self.max_iter_):
            ...
            if f.compute_output(X=X, y=y) < best_loss:
                best_w = f.weights
            w_sum += f.weights
        ...
        return best_w, w_sum, actual_iter
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        best_w, w_sum, actual_iter = self._best_w_calc(f, X, y)
