...
class AdaBoost(BaseEstimator):
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for t in range(0, min(T, self.iterations_)):
            b.append(self.weights_[t] * self.models_[t].predict(X))
        return np.sign(np.sum(b, axis=0))
    ...
    def partial_loss(self, X: np.ndarray, y: np.ndarray, T: int) -> float:
        ...
        y_prediction = self.partial_predict(X, T)
        return misclassification_error(y_true=y, y_pred=y_prediction)
