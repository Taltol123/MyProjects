...
class DecisionStump(BaseEstimator):
    ...
    def _find_threshold(self, values: np.ndarray, labels: np.ndarray, sign: int) -> Tuple[float, float]:
        ...
        threshold = -np.inf
        curr_error = np.where(labels * sign < 0, -labels * sign, 0).sum()
        min_error = curr_error

        for i in range(1, len(values) + 1):
            if i < len(values) and values[i - 1] == values[i]:
                continue
            elif np.sign(labels[i - 1]) == -sign:
                curr_error -= np.abs(labels[i - 1])
            else:
                curr_error += np.abs(labels[i - 1])

            if i < len(values) and curr_error < min_error:
                min_error = curr_error
                threshold = values[i]
            elif i == len(values) and curr_error < min_error:
                min_error = curr_error
                threshold = np.inf

        return threshold, min_error/len(values)
    ...
    def _loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        loss = misclassification_error(np.sign(y), predict)
        return loss
