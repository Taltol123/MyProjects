    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.lam_ == 0:
            if not self.include_intercept_:
                cofes = LinearRegression(include_intercept=False).fit(X, y).coefs_
            else:
                cofes = LinearRegression().fit(X, y).coefs_
            self.coefs_ = cofes
            ...
            XtX = X.T @ X
            lam_I = self.lam_ * I
            inverse = np.linalg.inv(XtX + lam_I)
            self.coefs_ = inverse @ X.T @ y
